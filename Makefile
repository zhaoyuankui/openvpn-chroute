...
all:
	...
	cp -Rf src/* output/bin
	cp -f conf/conf.sh output/conf
install:
	@if [ -f 'output/bin/install.sh' ]; then \
		cd output/bin; sh install.sh; cd - > /dev/null; \
	else \
	    mkdir -p ~/bin; \
	    if [ "`ls output/bin`" ]; then chmod +x output/bin/*;cp -pf output/bin/* ~/bin; fi; \
	fi
	@if [ -f 'output/conf/install.sh' ]; then \
		cd output/conf; sh install.sh; cd - > /dev/null; \
	else \
	    mkdir -p ~/bin/conf; \
...
uninstall:
	@if [ -f 'output/bin/uninstall.sh' ]; then \
		cd output/bin; sh uninstall.sh; cd - > /dev/null; \
	fi
	@if [ -f 'output/conf/uninstall.sh' ]; then \
		cd output/conf; sh uninstall.sh; cd - > /dev/null; \
	fi
